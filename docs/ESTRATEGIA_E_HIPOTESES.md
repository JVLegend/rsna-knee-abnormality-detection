...
- H-36 foi executada como uma família visual independente: CoAtNet RMLP 2 RW em 384 px, checkpoint Max-Span público CC0-1.0, cinco slots/64 fatias, crop físico de 140 mm, span 2–98% e 62 janelas sobrepostas. O kernel v1 T4 terminou `COMPLETE` em `25,4 s`, cobriu `3/3` estudos de teste, sem falhas, e gerou CSV 3×13 íntegro com SHA-256 `fc8b32d5964f54619ef358b8cf97291012806c16aea6fe8db799c1d3279829b7`. A submissão Notebook-only foi criada em 30/08 às 13:05 e está `PENDING`; ainda sem score.
...
9. H-36 concluiu a execução T4 com `3/3` estudos de teste e CSV íntegro. O
   ...
   submissão Notebook-only foi criada e está `PENDING`. Não promover antes do
   score público superar `0,899`.

### Próxima atualização

H-34 (`55856090`, `0,899`) é a referência protegida; H-36 está em processamento
após Submit como teste independente, e H-29 (`0,759`), H-27
