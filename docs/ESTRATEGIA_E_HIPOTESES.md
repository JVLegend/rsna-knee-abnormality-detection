...
- H-38 foi executada, validada e submetida como uma ablação residual: cinco folds DINOv3 ViT-S públicos (`CC0-1.0`) em 336 px, seis slots/16 fatias, crop de 130 mm e pooling `xcodex`, com fusão por rank de `20%` DINOv3 + `80%` CoAtNet H-36 (`CC0-1.0`). O kernel T4×2 terminou `COMPLETE`, cobriu `3/3` estudos e o CSV 3×13 passou schema, IDs únicos, finitude e faixa `[0,1]`; a saída Kaggle bateu byte a byte com a reconstrução local, SHA-256 `67265048d923bc060210651d598a5cfbd3029078dd4a556fd02ad337a9e00d0e`. A submissão Notebook-only de 31/08 às 10:06 BRT terminou `COMPLETE` com public score `0,929`, ganho `+0,001` sobre H-36; H-38 passa a ser o novo baseline de produção.
...
| H-38 | Um residual pequeno de DINOv3 preserva o ganho do CoAtNet H-36 e evita o excesso da fusão 50/50 do H-37 | `kaggle/rsna_knee_h38_dinov3_coatnet_residual/`; mesmos dois datasets públicos CC0-1.0, rank por alvo, peso DINOv3 `0,20` e CoAtNet `0,80`, sem `kernel_sources` | Rodar no T4, conferir `3/3`, schema e finitude; só promover se superar H-36 `0,928` | confirmada/promovida — kernel T4×2 `COMPLETE`, `3/3` estudos, CSV 3×13 íntegro; saída Kaggle e reconstrução local têm SHA-256 `67265048d923bc060210651d598a5cfbd3029078dd4a556fd02ad337a9e00d0e`; Notebook-only criada em 31/08 às 10:06 BRT, `COMPLETE`, public score `0,929` (`+0,001` vs H-36), novo baseline |
| H-41 | O checkpoint WideDense completo do mesmo ecossistema público supera H-38 como modelo primário | `dreaddevelopment/raptor-knee-widedense/raptor_ft_coatnet_v4_full.pt` e variante SWA; CoAtNet RMLP 2 RW 384, 336 px, crop 140 mm e janelas sobrepostas; fonte declarada `CC0-1.0`; full auditado com SHA-256 `89606f05849838529e1b4658d28fb049623205d9853371548403bca460361ded` e SWA com SHA-256 `d8bb0f8751b4bb65750257869ddc4c7a3c0cdfc6e62596fc68193919406c53eb` | Exigir gate gold independente, OOF condicionado por fold ou score Kaggle acima de `0,929`; não calibrar no leaderboard de apenas 3 estudos | **Refutada como primária no gate gold**: full `0,914097`, SWA `0,914483`, H-38 reconstruído `0,922466`; blend fixo H-38 80% + SWA 20% `0,922942`, IC95% do delta `[-0,002530; 0,003668]`; preservar apenas como complemento experimental |
...
- H-36 foi executada como família visual independente em
  ...
  sobre H-29; H-36 foi o baseline anterior, superado por H-38.
...
- H-38 foi preparada em
  ...
  O kernel T4 terminou `COMPLETE`, cobriu `3/3` estudos, gerou a mesma saída
  byte a byte e foi salvo como
  `submissions/submission_h38_dinov3_coatnet_residual.csv`. A submissão
  Notebook-only foi criada em 31/08 às 10:06 BRT com a versão 1 e o arquivo
  `submission.csv`; terminou `COMPLETE` com public score `0,929`, ganho
  `+0,001` sobre H-36, promovendo H-38 como novo baseline.
...
1. Não enviar outra cópia de H-28. Proteger H-38 (`0,929`) como referência
   operacional; H-36 (`0,928`), H-37 (`0,922`), H-34 (`0,899`), H-29
   (`0,759`), H-27 (`0,727`) e H-23 (`0,718`) ficam como fallbacks
   reproduzíveis.
...
3. Manter `report_hash` nos folds e não reabrir o peso gold `8` como prioridade:
   H-32 já marcou `0,720` no leaderboard. Só reabrir essa linha com uma hipótese
   nova, um controle peso `1` e um gate local independente; não escolher peso pelo
   leaderboard isolado.
...
7. Manter H-36 v1 como fallback independente: CoAtNet Max-Span CC0, atenção por
   alvo, crop físico e 62 janelas, com cobertura/schema/finitude conferidos. A
   submissão Notebook-only marcou `0,928`; H-38 a superou em `+0,001`.
8. Encerrar H-35 como bloqueada operacionalmente: o stack não recebeu os
   ...
   predições; manter somente fontes públicas permitidas e os CSVs já auditados
   como fallbacks.
9. H-36 concluiu a execução T4 com `3/3` estudos de teste e CSV íntegro. O
   ...
   submissão Notebook-only marcou `0,928`. H-36 é o fallback visual direto de
   H-38; qualquer próxima variante deve superar `0,929` sem misturar outputs
   privados.
...
11. H-38 concluiu a execução no T4 e foi submetida: é a primeira ablação
    pós-H-37 que reduz o braço DINOv3 a 20% e preserva 80% do H-36. O output
    Kaggle bateu byte a byte com a reconstrução local, passou os gates e a
    submissão Notebook-only terminou `COMPLETE` com public score `0,929`,
    `+0,001` sobre H-36; promover H-38 como novo baseline.

### Próxima atualização

H-38 (`0,929`) é a nova referência protegida; H-36 (`0,928`) e H-37 (`0,922`)
permanecem como fallbacks, com H-38 usando residual DINOv3 `20%`;
H-34 (`55856090`, `0,899`),
H-29 (`0,759`), H-27
...
## Atualização de triagem de fontes — 02/09/2026: WideDense full e H-41

### Fatos verificados

- A página pública do kernel [Dread Development — Twelve Findings from a Single Model](https://www.kaggle.com/code/dreaddevelopment/knee-mri-twelve-findings-from-a-single-model) informa public score `0,924` e descreve um único CoAtNet WideDense com crop de `140 mm`, resolução `336 px` e janelas sobrepostas. É uma referência pública, não um resultado nosso.
- O dataset [raptor-knee-widedense](https://www.kaggle.com/datasets/dreaddevelopment/raptor-knee-widedense) declara `CC0-1.0`. Baixamos `raptor_ft_coatnet_v4_full.pt` no HD externo: `292.829.402` bytes, arquitetura `coatnet_rmlp_2_rw_384.sw_in12k_ft_in1k`, resolução `384`, 12 alvos e SHA-256 `89606f05849838529e1b4658d28fb049623205d9853371548403bca460361ded`.
- A leitura do checkpoint é íntegra e registra `epoch=12`, `gold_auc=0,916749` e `src=timm-pretrained`. Esse `gold_auc` é a métrica armazenada pelo autor; não é uma validação OOF nossa.
- A auditoria atual de supervisão fraca continua favorecendo `targetwise_teacher` (`0,899120`) sobre consenso por ranking (`0,896616`), Steven v4 (`0,892707`) e Pilkwang (`0,870040`) nos 58 estudos oficiais. Não apareceu uma fonte nova que justifique substituir o teacher.
- A triagem de licenças separou fontes `CC0-1.0`/Apache 2.0, adequadas à linha equity-free do projeto, de pesos `CC-BY-NC-SA-4.0`, `other` ou `unknown`, que ficam fora até haver autorização e revisão específica. A licença CC0 do arquivo não transforma claims de leaderboard em evidência clínica.

### Decisão H-41

H-41 **não será executada como modelo primário** nesta rodada. O melhor resultado público conhecido do artefato (`0,924`) já fica abaixo do H-38 (`0,929`), e o H-40 residual mostrou que o complemento WideDense não alterou o baseline de maneira útil: terminou byte a byte igual ao H-36 (`0,928`). Isso é uma triagem de custo baseada em evidência pública, não uma refutação experimental no nosso kernel.

O checkpoint permanece armazenado para o único uso que ainda faz sentido: um blend condicionado por fold/OOF com H-36/H-38, validado em estudos anotados independentes antes de qualquer nova submissão. Não ajustaremos esse blend usando as três linhas públicas do teste.

## Gate independente no gold — 02/09/2026: H-41 WideDense e residual DINOv3

### Contrato e evidência

- O gate usou os `58` estudos de treino com os `12` rótulos oficiais completos e `336` séries de metadados; nenhum rótulo entrou na inferência. A reprodução do WideDense usou cinco slots, crop físico de `140 mm`, `384 px`, `64` fatias e `62` janelas, conforme o pipeline H-40/H-41.
- O checkpoint full marcou macro-AUC `0,914097`; o checkpoint SWA exato usado pelo H-40 marcou `0,914483`. Os relatórios completos estão em `reports/widedense_gold_20260902.json` e `reports/widedense_swa_gold_20260902.json`.
- O H-36 independente marcou `0,919929`. A reconstrução local do H-38 — `80%` H-36 por rank + `20%` DINOv3 base por rank — marcou `0,922466`, coerente com o fato de H-38 ser o baseline público `0,929`.
- O blend fixo H-38 `80%` + WideDense SWA `20%` marcou `0,922942` (`+0,000476` no gold). Nos pesos fixos `15%` e `30%`, marcou `0,922797` e `0,922765`. O bootstrap pareado de `2.000` reamostragens deu, respectivamente, delta médio `+0,000324` com IC95% `[-0,002112; 0,002944]`, `+0,000464` com IC95% `[-0,002530; 0,003668]` e `+0,000308` com IC95% `[-0,004371; 0,005177]`.
- A auditoria do residual DINOv3 sobre H-36 também manteve o máximo local em torno de `20%`: pesos `5/10/15/20/25/30%` deram `0,920423/0,922067/0,922241/0,922466/0,921481/0,920971`. O resultado do gold é diagnóstico, não OOF; o peso `20%` permanece porque também foi o único que ganhou no leaderboard público H-38.
