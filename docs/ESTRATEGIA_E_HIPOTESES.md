...
- H-37 foi preparada como uma família independente e auditável para testar diversidade de representação: cinco folds DINOv3 ViT-S públicos da Mattia Angeli (`CC0-1.0`) em 336 px, seis slots/16 fatias, crop de 130 mm e pooling `xcodex`, fundidos por rank com o CoAtNet Max-Span H-36 (`CC0-1.0`). O kernel não depende de outputs privados, usa somente `mattiaangeli/knee-mri-fold-weights` e `dreaddevelopment/raptor-knee-maxspan`, e permanece aguardando execução no T4. A referência pública do braço DINOv3 é `0,920` atual / `0,922` best; isso é evidência de direção, não resultado nosso.
...
- H-37 está preparada em
  ...
  H-36. A fusão usa média 50/50 dos ranks por alvo e não monta nenhum output
  privado. O braço DINOv3 tem score público declarado `0,920` atual / `0,922`
  best no notebook de referência, mas ainda não há execução nossa; o gate é
  `3/3` estudos, schema/valores finitos e score acima de `0,928` para promoção.
...
10. H-37 é a próxima execução autorizada: DINOv3 público independente + H-36
    CoAtNet, rank por alvo, sem `kernel_sources`. Não reutilizar o CSV H-36 como
    input: ele deve ser inferido novamente pelo próprio kernel para manter a
    ablação e a linhagem auditáveis.

### Próxima atualização

H-36 (`0,928`) é a nova referência protegida; H-37 é a próxima candidata;
H-34 (`55856090`, `0,899`),
H-29 (`0,759`), H-27
