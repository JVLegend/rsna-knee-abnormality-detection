...
| 09/08/2026 (UTC 09/08) | `kaggle_kernel_v3_weak_visual_v6` | Rodar a v3 no kernel multi-view histórico, preservando o anexo de pesos offline; limiar `0,85` e peso weak máximo `0,10` | Kernel `jvlegend/rsna-knee-abnormality-detection-v2-multiview`, versão 6, `RUNNING`; o novo kernel isolado não montou os pesos, então foi mantida a configuração CPU/offline já validada | — | Aguardar execução; promover somente após CSV íntegro e score futuro superior a `0,635` |
