...
def test_subset_can_exclude_an_existing_manifest() -> None:
    train = pd.DataFrame(
        [
            {KEY_COLUMN: "study-positive", "PatientSex": "F", "Report": "", **{target: 1 for target in TARGET_COLUMNS}},
            {KEY_COLUMN: "study-negative", "PatientSex": "M", "Report": "", **{target: 0 for target in TARGET_COLUMNS}},
    ...
    manifest = select_subset(train, series, per_class=1, max_studies=2, excluded_studies={"study-positive"})

    assert {entry["study_uid"] for entry in manifest["studies"]} == {"study-negative"}
