#!/usr/bin/env python3
"""Candidata v3 multi-view com weak labels públicos CC0.
...
ele não clona o repositório, não instala pacotes e não usa internet. Os labels
Steven v4 são neutralizados nos casos que o v2 marcou como não abordados.
"""
...
def _external_teacher(train: pd.DataFrame, requested_dir: Path | None = None) -> tuple[pd.DataFrame, Path]:
    label_dir = _find_external_label_dir(requested_dir)
    v2 = pd.read_csv(label_dir / EXTERNAL_LABEL_V2_FILENAME)
    v4 = pd.read_csv(label_dir / EXTERNAL_LABEL_V4_FILENAME)
    for frame, name in ((v2, "v2"), (v4, "v4")):
    ...
    v2 = v2.set_index(KEY_COLUMN)
    v4 = v4.set_index(KEY_COLUMN)
    ids = train[KEY_COLUMN].astype(str)
    result = pd.DataFrame({KEY_COLUMN: ids.to_numpy()})
    for target in TARGET_COLUMNS:
        if target not in v2.columns or target not in v4.columns:
            raise ValueError(f"Labels externos sem o alvo {target!r}.")
        raw = pd.to_numeric(ids.map(v4[target]), errors="coerce")
        v2_raw = pd.to_numeric(ids.map(v2[target]), errors="coerce")
        # No v2, 0,5 significa que o relatório não abordou o achado. O v4
        # combina fontes e pode transformar esse estado em 0,25; neutralizar
        # antes do treino evita fabricar negativos.
        not_addressed = v2_raw.sub(0.5).abs().le(1e-9)
        score = raw.mask(not_addressed, 0.5).clip(0.0, 1.0)
        result[target] = score.to_numpy()
        result[f"{target}__confidence"] = score.sub(0.5).abs().mul(2.0).to_numpy()
    result.attrs["label_dir"] = str(label_dir)
    result.attrs["label_files"] = [EXTERNAL_LABEL_V2_FILENAME, EXTERNAL_LABEL_V4_FILENAME]
    return result, label_dir
...
def run(
    ...
    if weak_visual:
        teacher, label_dir = _external_teacher(train, external_labels_dir)
