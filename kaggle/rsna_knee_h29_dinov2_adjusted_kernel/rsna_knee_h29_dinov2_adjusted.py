#!/usr/bin/env python3
"""H-29 — crop físico 130 mm/336 px + DINOv2-S ajustado. #RSNA #Kaggle #Dados

Esta variante preserva o teacher target-wise, o blend texto-imagem, labels weak
suaves e a cabeça visual separada por Sagittal, Coronal e Axial da H-27, mas
troca a representação por DINOv2-S oficial MetaResearch. Cada slab usa crop
físico central de aproximadamente 130 mm renderizado em 336 px; o backbone é
ajustado em uma época nos últimos blocos e o head auxiliar multilabel é
descartado depois.
...
O modelo oficial é anexado pelo Kaggle Models com licença Apache 2.0; o
entrypoint permanece autocontido para Notebook-only e não usa internet.
O entrypoint é autocontido para Notebook-only: não clona o repositório, não
instala pacotes e não usa internet.
"""
...
PLANE_KEYS = (*PLANE_NAMES, "Unknown")
LEXICON = {
...
FINE_TUNE_EPOCHS = int(os.environ.get("RSNA_FINE_TUNE_EPOCHS", "1"))
FINE_TUNE_LR = float(os.environ.get("RSNA_FINE_TUNE_LR", "1e-5"))
FINE_TUNE_TAIL_BLOCKS = int(os.environ.get("RSNA_FINE_TUNE_TAIL_BLOCKS", "1"))
FINE_TUNE_BATCH_SIZE = int(os.environ.get("RSNA_FINE_TUNE_BATCH_SIZE", "64"))
FINE_TUNE_MAX_STUDIES = int(os.environ.get("RSNA_FINE_TUNE_MAX_STUDIES", "0"))
...
def _preferred_series_uids_by_plane(
    series: pd.DataFrame | dict[str, pd.DataFrame],
    study_uid: str,
) -> dict[str, str]:
    """Escolhe uma série por plano, preservando fallback explícito."""
    ...
    for plane_name in PLANE_NAMES:
        ...
        if not subset.empty:
            selected[plane_name] = str(subset.iloc[0]["SeriesInstanceUID"])
    if selected:
        return selected
    fallback = candidates.sort_values(["_fluid", "_fat", "SeriesInstanceUID"], ascending=[False, False, True])
    return {"Unknown": str(fallback.iloc[0]["SeriesInstanceUID"])}
...
def study_images_by_plane(
    ...
    fast_preprocess: bool = False,
    intensity_window: str = "slice",
) -> tuple[dict[str, list[np.ndarray]], bool]:
    """Extrai views mantendo a identidade do plano e a máscara de presença."""

    images_by_plane: dict[str, list[np.ndarray]] = {plane: [] for plane in PLANE_KEYS}
    for plane, series_uid in _preferred_series_uids_by_plane(series, study_uid).items():
        series_images, valid = _series_image(
            data_dir,
            split,
            study_uid,
        ...
        if valid:
            images_by_plane.setdefault(plane, []).extend(series_images)
    return images_by_plane, any(images_by_plane.values())
...
def _load_encoder(device: str) -> tuple[torch.nn.Module, str, Path]:
    ...
    return model.to(device), device, weights_path
...
def _fine_tune_encoder(
    ...
    weak_targets: dict[str, tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]],
    *,
    device: str,
    slice_profile: str,
    ...
    """Ajusta apenas o final do B0 com um head auxiliar multilabel.

    Cada slab recebe o rótulo do estudo. Para estudos weak, o alvo é a
    probabilidade do teacher e o peso é o mesmo usado no treino visual final;
    alvos não abordados ficam mascarados. O head é descartado antes da
    extração dos embeddings, isolando a mudança na representação.
    """

    if epochs < 1 or learning_rate <= 0 or tail_blocks < 1 or batch_size < 1:
        raise ValueError("Parâmetros de fine-tuning inválidos.")
    if max_studies < 0:
        raise ValueError("max_studies precisa ser >= 0.")

    train_frame = train_frame.reset_index(drop=True)
    study_count = len(train_frame) if max_studies == 0 else min(max_studies, len(train_frame))
    if study_count == 0:
        raise ValueError("Fine-tuning sem estudos de treino.")
    train_frame = train_frame.iloc[:study_count].reset_index(drop=True)

    labels = np.full((study_count, len(TARGET_COLUMNS)), np.nan, dtype=np.float32)
    mask = np.zeros_like(labels, dtype=np.float32)
    weights = np.zeros_like(labels, dtype=np.float32)
    for target_index, target in enumerate(TARGET_COLUMNS):
        gold_values = pd.to_numeric(train_frame[target], errors="coerce").to_numpy(dtype=float)
        source_labels, source_weights, included, soft_probabilities = weak_targets[target]
        source_labels = np.asarray(source_labels, dtype=float)[:study_count]
        source_weights = np.asarray(source_weights, dtype=float)[:study_count]
        included = np.asarray(included, dtype=bool)[:study_count]
        soft_probabilities = np.asarray(soft_probabilities, dtype=float)[:study_count]
        gold = np.isfinite(gold_values)
        weak = (~gold) & included & np.isfinite(soft_probabilities)
        labels[:, target_index] = np.where(
            gold,
            gold_values,
            np.where(weak, soft_probabilities, source_labels),
        ).astype(np.float32)
        mask[:, target_index] = (gold | weak).astype(np.float32)
        weights[:, target_index] = np.where(gold, 1.0, source_weights).astype(np.float32)

    children = list(model.features.children())
    tail_blocks = min(tail_blocks, len(children) - 1)
    prefix = torch.nn.Sequential(*children[:-tail_blocks]).to(device).eval()
    tail = torch.nn.Sequential(*children[-tail_blocks:]).to(device)
    for parameter in prefix.parameters():
        parameter.requires_grad = False
    for parameter in tail.parameters():
        parameter.requires_grad = True
    tail.train()
    head = torch.nn.Linear(1280, len(TARGET_COLUMNS)).to(device)
    optimizer = torch.optim.AdamW(
        [parameter for parameter in tail.parameters() if parameter.requires_grad]
    ...
    series_index = _series_index(train_series)
    records = [(str(row[KEY_COLUMN]), series_index) for _, row in train_frame.iterrows()]
    history: list[float] = []
    views_seen = 0
    ...
    for epoch in range(epochs):
        ...
        def flush() -> None:
            ...
            if not batch_images:
                return
            x = torch.from_numpy(np.stack(batch_images).astype(np.float32) / 255.0)
            x = x.to(device).sub(mean).div(std)
            owner_index = np.asarray(batch_owners, dtype=np.int64)
            target = torch.from_numpy(np.nan_to_num(labels[owner_index], nan=0.0)).to(device)
            target_mask = torch.from_numpy(mask[owner_index]).to(device)
            target_weight = torch.from_numpy(weights[owner_index]).to(device)
            with torch.no_grad():
                hidden = prefix(x)
            features = tail(hidden)
            pooled = model.avgpool(features).flatten(1)
            logits = head(pooled)
            element_loss = loss_function(logits, target)
            effective_weight = target_mask * target_weight
            denominator = effective_weight.sum().clamp_min(1.0)
            loss = (element_loss * effective_weight).sum() / denominator
            optimizer.zero_grad(set_to_none=True)
            loss.backward()
            torch.nn.utils.clip_grad_norm_(list(tail.parameters()) + list(head.parameters()), max_norm=1.0)
            optimizer.step()
            epoch_losses.append(float(loss.detach().cpu()))
            views_seen += len(batch_images)
            batch_images.clear()
            batch_owners.clear()

        for owner, (study_uid, series) in enumerate(records):
            images, valid = study_images(
                data_dir,
                "train",
                study_uid,
                series,
            ...
            batch_owners.extend([owner] * len(images))
            if len(batch_images) >= batch_size:
                flush()
            if (owner + 1) % 25 == 0 or owner + 1 == len(records):
                print(
                    f"fine_tune epoch={epoch + 1}/{epochs} "
                    f"studies={owner + 1}/{len(records)} views={views_seen + len(batch_images)}"
                )
        flush()
        epoch_loss = float(np.mean(epoch_losses)) if epoch_losses else float("nan")
        history.append(epoch_loss)
        print(
            f"fine_tune_done epoch={epoch + 1}/{epochs} loss={epoch_loss:.6f} "
            f"batches={len(epoch_losses)} views={views_seen} tail_blocks={tail_blocks}"
        )
    ...
    return {
        ...
        "augmentation": "none; no horizontal flip",
    }
...
def visual_embeddings(
    ...
    fast_preprocess: bool = False,
    intensity_window: str = "slice",
    fine_tune_targets: dict[str, tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]] | None = None,
    fine_tune_epochs: int = FINE_TUNE_EPOCHS,
    fine_tune_lr: float = FINE_TUNE_LR,
    fine_tune_tail_blocks: int = FINE_TUNE_TAIL_BLOCKS,
    fine_tune_batch_size: int = FINE_TUNE_BATCH_SIZE,
    fine_tune_max_studies: int = FINE_TUNE_MAX_STUDIES,
) -> tuple[
    np.ndarray,
    np.ndarray,
    dict[str, object],
    ...
    if intensity_window not in {"slice", "series"}:
        raise ValueError(f"Janela de intensidade desconhecida: {intensity_window}")
    model, device, weights_path = _load_encoder(device)
    fine_tune_meta: dict[str, object] = {"enabled": False}
    if fine_tune_targets is not None:
        fine_tune_meta = _fine_tune_encoder(
            model,
            data_dir,
            train_labeled,
            train_series,
            ...
            tail_blocks=fine_tune_tail_blocks,
            batch_size=fine_tune_batch_size,
            max_studies=fine_tune_max_studies,
        )
    ...
    study_embeddings_by_plane: list[dict[str, list[np.ndarray]]] = [
        {plane: [] for plane in PLANE_KEYS} for _ in records
    ]
    mean = IMAGE_MEAN.to(device)
    std = IMAGE_STD.to(device)
    ...
    def flush() -> None:
        ...
        for owner, plane, value in zip(image_owners, image_planes, values, strict=True):
            study_embeddings[owner].append(value)
            study_embeddings_by_plane[owner].setdefault(plane, []).append(value)
        images.clear()
        image_owners.clear()
        image_planes.clear()
    ...
    for position, (split, row, series) in enumerate(records, start=1):
        owner = position - 1
        study_images_for_row, valid = study_images_by_plane(
            data_dir,
            split,
            str(row[KEY_COLUMN]),
            series,
        ...
        for plane, plane_images in study_images_for_row.items():
    ...
    meta = {
        ...
        "plane_coverage_train": {
            plane: int(sum(bool(study_embeddings_by_plane[index].get(plane)) for index in range(train_count)))
            for plane in PLANE_NAMES
        },
        "plane_coverage_test": {
            plane: int(sum(bool(study_embeddings_by_plane[index].get(plane)) for index in range(train_count, len(records))))
            for plane in PLANE_NAMES
        },
        ...
        "fine_tune": fine_tune_meta,
    }
    return (
        matrix[:train_count],
        matrix[train_count:],
        meta,
        study_embeddings[:train_count],
        study_embeddings[train_count:],
        study_embeddings_by_plane[:train_count],
        study_embeddings_by_plane[train_count:],
    )
...
def _fit_target_plane_view_model(
    target: str,
    train_views_by_plane: list[dict[str, list[np.ndarray]]],
    test_views_by_plane: list[dict[str, list[np.ndarray]]],
    labels: np.ndarray,
    fit_mask: np.ndarray,
    sample_weights: np.ndarray | None,
    target_pooling: str | None = None,
    ...
    """Treina uma cabeça visual por plano e agrega somente planos presentes."""

    if len(train_views_by_plane) != len(labels) or len(train_views_by_plane) != len(fit_mask):
        raise ValueError("Os mapas de plano precisam coincidir com labels/fit_mask.")
    if len(test_views_by_plane) == 0:
        return np.empty(0, dtype=float)
    per_plane: dict[str, np.ndarray] = {}
    for plane in PLANE_NAMES:
        train_views = [study.get(plane, []) for study in train_views_by_plane]
        test_views = [study.get(plane, []) for study in test_views_by_plane]
        per_plane[plane] = _fit_target_view_model(
            target,
            train_views,
            test_views,
            labels,
    ...
    predictions: list[float] = []
    for index, study in enumerate(test_views_by_plane):
        present = [per_plane[plane][index] for plane in PLANE_NAMES if study.get(plane)]
        predictions.append(float(np.mean(present)) if present else 0.5)
    return np.clip(np.asarray(predictions, dtype=float), 1e-6, 1 - 1e-6)
...
def run(
    ...
    if slice_profile not in SLICE_PROFILES:
        raise ValueError(f"Perfil de fatias desconhecido: {slice_profile}")
    if view_pooling not in {"mean", "max", "topk", "target", "plane_target"}:
    ...
    if fine_tune_encoder and not weak_visual:
        raise ValueError("H-23 exige weak_visual=True para criar o alvo auxiliar.")
    if fine_tune_epochs < 1 or fine_tune_lr <= 0 or fine_tune_tail_blocks < 1 or fine_tune_batch_size < 1:
        raise ValueError("Parâmetros de fine-tuning inválidos.")
    if fine_tune_max_studies < 0:
        raise ValueError("fine_tune_max_studies precisa ser >= 0.")
    ...
    embedding_pooling = "mean" if view_pooling in {"target", "plane_target"} else view_pooling
    (
        train_visual,
        test_visual,
        visual_meta,
        ...
        fast_preprocess=fast_preprocess,
        intensity_window=intensity_window,
        fine_tune_targets=weak_targets if fine_tune_encoder else None,
        fine_tune_epochs=fine_tune_epochs,
        fine_tune_lr=fine_tune_lr,
        fine_tune_tail_blocks=fine_tune_tail_blocks,
        fine_tune_batch_size=fine_tune_batch_size,
        fine_tune_max_studies=fine_tune_max_studies,
    )
    ...
    for target in TARGET_COLUMNS:
        if weak_visual:
            ...
            sample_weights = None
        if view_pooling == "plane_target":
            visual[target] = _fit_target_plane_view_model(
                target,
                train_views_by_plane,
                test_views_by_plane,
                labels_for_views,
    ...
    print(
        ...
        f"teacher_profile={teacher_profile} fine_tune_encoder={fine_tune_encoder}"
    )
    if targetwise:
...
H29_IMAGE_SIZE = 336
H29_CROP_MM = 130.0
H29_EMBEDDING_DIM = 384


def _h29_find_model_dir(requested: Path | None = None) -> Path:
    candidates: list[Path] = []
    if requested is not None:
        candidates.append(requested.expanduser())
    ...
    raise FileNotFoundError(
        ...
        + ", ".join(str(path) for path in candidates[:40])
    )


def _h29_load_model(requested_model_dir: Path | None, device: str) -> tuple[torch.nn.Module, str, Path]:
    from transformers import AutoModel

    device = _resolve_device(device)
    model_dir = _h29_find_model_dir(requested_model_dir)
    model = AutoModel.from_pretrained(model_dir, local_files_only=True).to(device)
    model.eval()
    hidden_size = int(getattr(model.config, "hidden_size", 0))
    if hidden_size != H29_EMBEDDING_DIM:
        raise ValueError(f"Esperava DINOv2-S com hidden_size={H29_EMBEDDING_DIM}; recebi {hidden_size}.")
    return model, device, model_dir


def _h29_forward(model: torch.nn.Module, pixel_values: torch.Tensor):
    try:
        return model(pixel_values=pixel_values, interpolate_pos_encoding=True)
    except TypeError:
        return model(pixel_values=pixel_values)


def _h29_crop_center(normalized: np.ndarray, pixel_spacing: object, crop_mm: float) -> np.ndarray:
    if crop_mm <= 0:
    ...
    return normalized[row_start : row_start + crop_rows, col_start : col_start + crop_cols]


def _h29_read_slice(path: Path) -> tuple[np.ndarray, object]:
    dataset = pydicom.dcmread(path, force=True)
    pixels = dataset.pixel_array.astype(np.float32)
    slope = _number(getattr(dataset, "RescaleSlope", 1.0)) or 1.0
    intercept = _number(getattr(dataset, "RescaleIntercept", 0.0)) or 0.0
    ...
    if str(getattr(dataset, "PhotometricInterpretation", "")) == "MONOCHROME1":
        pixels = float(np.max(pixels)) - pixels
    return pixels, getattr(dataset, "PixelSpacing", None)
...
def _series_image(
    ...
    study_uid: str,
    series_uid: str,
    size: int = H29_IMAGE_SIZE,
    slice_profile: str = SLICE_PROFILE,
    fast_preprocess: bool = False,
    intensity_window: str = "series",
) -> tuple[list[np.ndarray], bool]:
    """Extrai slabs 2.5D usando crop físico central, mantendo fallback seguro."""
    ...
    for path in paths:
        try:
            header = pydicom.dcmread(
                ...
                force=True,
                specific_tags=["InstanceNumber", "ImagePositionPatient"],
            ) if fast_preprocess else pydicom.dcmread(path, stop_before_pixels=True, force=True)
            records.append((path, header))
        except Exception:
            continue
    if not records:
    ...
    percentile_stride = 4 if fast_preprocess else 1
    raw_cache: dict[int, np.ndarray] = {}
    spacing_cache: dict[int, object] = {}
    try:
        for index in used_indices:
            raw_cache[index], spacing_cache[index] = _h29_read_slice(records[index][0])
    except Exception:
        return [], False
    series_bounds = (
        _series_window_bounds(list(raw_cache.values()), percentile_stride)
        if intensity_window == "series"
        else None
    )
    resized_cache: dict[int, np.ndarray] = {}
    ...
    def resized_slice(index: int) -> np.ndarray:
        if index not in resized_cache:
            pixels = raw_cache[index]
            bounds = series_bounds or _series_window_bounds([pixels], percentile_stride)
            normalized = _normalize_slice(pixels, percentile_stride, bounds=bounds)
            cropped = _h29_crop_center(normalized, spacing_cache.get(index), H29_CROP_MM)
            image = Image.fromarray(np.rint(np.clip(cropped, 0.0, 1.0) * 255).astype(np.uint8), mode="L")
            resized_cache[index] = np.asarray(
                image.resize((size, size), resample=Image.Resampling.BILINEAR),
                dtype=np.uint8,
            )
    ...
    if slice_profile == "quantile3":
        try:
            return [np.stack([resized_slice(index) for index in centers], axis=0)], True
        except Exception:
            return [], False
    slabs: list[np.ndarray] = []
    for center in centers:
...
def _h29_amp(device: str):
    if device == "cuda":
        return torch.autocast(device_type="cuda", dtype=torch.float16)
    return nullcontext()


def _h29_tensor(images: list[np.ndarray], device: str) -> torch.Tensor:
    values = torch.from_numpy(np.stack(images).astype(np.float32) / 255.0).to(device)
    return values.sub(IMAGE_MEAN.to(device)).div(IMAGE_STD.to(device))


def _h29_dino_layers(model: torch.nn.Module) -> list[torch.nn.Module]:
    encoder = getattr(model, "encoder", None)
    for name in ("layer", "layers"):
        layers = getattr(encoder, name, None)
        if layers is not None and len(layers):
            return list(layers)
    raise AttributeError("Não encontrei os blocos encoder.layer/layers do DINOv2.")
...
def _h29_fine_tune(
    ...
    *,
    ...
    layers = _h29_dino_layers(model)
    tail_blocks = min(tail_blocks, len(layers))
    trainable_modules = layers[-tail_blocks:]
    ...
    for name in ("layernorm", "norm"):
        ...
        if module is not None:
            for parameter in module.parameters():
                parameter.requires_grad_(True)
    head = torch.nn.Linear(H29_EMBEDDING_DIM, len(TARGET_COLUMNS)).to(device)
    ...
    for epoch in range(epochs):
        ...
        def flush() -> None:
            ...
            if not batch_images:
                return
            x = _h29_tensor(batch_images, device)
            owner_index = np.asarray(batch_owners, dtype=np.int64)
            target = torch.from_numpy(np.nan_to_num(labels[owner_index], nan=0.0)).to(device)
            target_mask = torch.from_numpy(mask[owner_index]).to(device)
            target_weight = torch.from_numpy(weights[owner_index]).to(device)
            optimizer.zero_grad(set_to_none=True)
            with _h29_amp(device):
                output = _h29_forward(model, x)
                logits = head(output.last_hidden_state[:, 0].float())
                element_loss = loss_function(logits, target)
                effective_weight = target_mask * target_weight
                denominator = effective_weight.sum().clamp_min(1.0)
                loss = (element_loss * effective_weight).sum() / denominator
        ...
        for owner, (study_uid, series) in enumerate(records):
            images_by_plane, valid = study_images_by_plane(
                data_dir,
                "train",
                study_uid,
                series,
            ...
            if not valid:
                continue
            for plane in PLANE_NAMES:
                batch_images.extend(images_by_plane.get(plane, []))
                batch_owners.extend([owner] * len(images_by_plane.get(plane, [])))
            if len(batch_images) >= batch_size:
                flush()
            if (owner + 1) % 25 == 0 or owner + 1 == len(records):
                print(
                    f"dino_fine_tune epoch={epoch + 1}/{epochs} "
                    f"studies={owner + 1}/{len(records)} views={views_seen + len(batch_images)}",
                    flush=True,
                )
        flush()
        ...
        history.append(epoch_loss)
        print(
            f"dino_fine_tune_done epoch={epoch + 1}/{epochs} loss={epoch_loss:.6f} "
            f"batches={len(epoch_losses)} views={views_seen} tail_blocks={tail_blocks}",
            flush=True,
        )
    model.eval()
    ...
    return {
        ...
        "views": views_seen,
        "loss_history": history,
        "image_size": H29_IMAGE_SIZE,
        "crop_mm": H29_CROP_MM,
        "augmentation": "none; no horizontal flip",
    }


def _h29_pool(values: list[np.ndarray], pooling: str) -> np.ndarray:
    if not values:
        return np.zeros(H29_EMBEDDING_DIM, dtype=np.float32)
    matrix = np.stack(values).astype(np.float32)
    if pooling == "mean":
        return matrix.mean(axis=0)
    if pooling == "max":
    ...
    if pooling == "topk":
        k = max(1, int(np.ceil(matrix.shape[0] * 0.25)))
        return np.sort(matrix, axis=0)[-k:].mean(axis=0)
    raise ValueError(f"Pooling de embedding DINOv2 desconhecido: {pooling}")
...
def visual_embeddings(
    ...
    batch_size: int = 16,
    device: str = "auto",
    slice_profile: str = "adjacent3",
    pooling: str = "mean",
    fast_preprocess: bool = False,
    intensity_window: str = "series",
    fine_tune_targets: dict[str, tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]] | None = None,
    fine_tune_epochs: int = 1,
    fine_tune_lr: float = 2e-6,
    fine_tune_tail_blocks: int = 6,
    fine_tune_batch_size: int = 16,
    fine_tune_max_studies: int = 0,
) -> tuple[
    np.ndarray,
    np.ndarray,
    dict[str, object],
    ...
    if batch_size < 1 or slice_profile not in SLICE_PROFILES:
        raise ValueError("batch_size/perfil de fatias inválido.")
    if pooling not in {"mean", "max", "topk"}:
        raise ValueError(f"Pooling de embedding desconhecido: {pooling}")
    if intensity_window not in {"slice", "series"}:
        raise ValueError(f"Janela de intensidade desconhecida: {intensity_window}")
    model, device, model_dir = _h29_load_model(None, device)
    fine_tune_meta: dict[str, object] = {"enabled": False}
    if fine_tune_targets is not None:
        fine_tune_meta = _h29_fine_tune(
            model,
            data_dir,
            train_labeled,
            train_series,
    ...
    def flush() -> None:
        if not images:
            return
        x = _h29_tensor(images, device)
        with torch.inference_mode():
            with _h29_amp(device):
                values = _h29_forward(model, x).last_hidden_state[:, 0].float().cpu().numpy()
        values = np.asarray(values, dtype=np.float32)
        for owner, plane, value in zip(image_owners, image_planes, values):
            study_embeddings[owner].append(value)
            study_embeddings_by_plane[owner].setdefault(plane, []).append(value)
        images.clear()
        image_owners.clear()
    ...
    for position, (split, row, series) in enumerate(records, start=1):
        ...
        for plane, plane_images in images_by_plane.items():
            for image in plane_images:
                images.append(image)
                image_owners.append(owner)
                image_planes.append(plane)
        ...
        if position % 25 == 0 or position == len(records):
            print(
                f"dino_visual={position}/{len(records)} valid={sum(valid_flags)} "
                f"views={sum(len(values) for values in study_embeddings)}",
                flush=True,
            )
    flush()
    matrix = np.stack([_h29_pool(values, pooling) for values in study_embeddings]).astype(np.float32)
    valid = np.asarray(valid_flags, dtype=bool)
    matrix[~valid] = 0.0
    train_count = len(train_labeled)
    meta = {
        ...
        "encoder": "dinov2-small",
        "embedding_dim": H29_EMBEDDING_DIM,
        "study_count_train": train_count,
        "study_count_test": len(test),
        "valid_train": int(valid[:train_count].sum()),
        "valid_test": int(valid[train_count:].sum()),
...
def main() -> None:
    ...
    parser.add_argument("--view-pooling", choices=("mean", "max", "topk", "target", "plane_target"), default=VIEW_POOLING)
    ...
    parser.add_argument("--fine-tune-encoder", dest="fine_tune_encoder", action="store_true", default=FINE_TUNE_ENCODER)
    parser.add_argument("--no-fine-tune-encoder", dest="fine_tune_encoder", action="store_false")
    ...
    parser.add_argument("--fine-tune-tail-blocks", type=int, default=FINE_TUNE_TAIL_BLOCKS)
    parser.add_argument("--fine-tune-batch-size", type=int, default=FINE_TUNE_BATCH_SIZE)
    parser.add_argument("--fine-tune-max-studies", type=int, default=FINE_TUNE_MAX_STUDIES)
    ...
    run(
        ...
        fine_tune_tail_blocks=args.fine_tune_tail_blocks,
        fine_tune_batch_size=args.fine_tune_batch_size,
        fine_tune_max_studies=args.fine_tune_max_studies,
    )


def h29_main() -> None:
    parser = argparse.ArgumentParser(description="H-29 RSNA Knee: crop físico + DINOv2-S ajustado")
    parser.add_argument("--data-dir", default=os.environ.get("RSNA_DATA_DIR", "/kaggle/input/rsna-knee-abnormality-detection"))
    parser.add_argument("--model-dir", default=os.environ.get("RSNA_DINOV2_MODEL_DIR"))
    parser.add_argument("--visual-weight", type=float, default=0.4)
    parser.add_argument("--targetwise", action="store_true", default=False)
    parser.add_argument("--batch-size", type=int, default=16)
    parser.add_argument("--device", choices=("auto", "cpu", "cuda", "mps"), default="auto")
    parser.add_argument("--weak-visual", action="store_true", default=True)
    parser.add_argument("--weak-threshold", type=float, default=0.85)
    parser.add_argument("--weak-sample-weight", type=float, default=0.10)
    parser.add_argument("--weak-label-mode", choices=("hard", "soft"), default="soft")
    parser.add_argument("--external-labels-dir", default=os.environ.get("RSNA_EXTERNAL_LABELS_DIR"))
    parser.add_argument("--slice-profile", choices=tuple(SLICE_PROFILES), default="adjacent3")
    parser.add_argument("--view-pooling", choices=("mean", "max", "topk", "target", "plane_target"), default="plane_target")
    parser.add_argument("--teacher-profile", choices=("steven_v4", "targetwise"), default="targetwise")
    parser.add_argument("--target-pooling", choices=("default", "mean", "max", "topk"), default="default")
    parser.add_argument("--intensity-window", choices=("slice", "series"), default="series")
    parser.add_argument("--fast-preprocess", action="store_true")
    parser.add_argument("--fine-tune-encoder", dest="fine_tune_encoder", action="store_true", default=True)
    parser.add_argument("--no-fine-tune-encoder", dest="fine_tune_encoder", action="store_false")
    parser.add_argument("--fine-tune-epochs", type=int, default=1)
    parser.add_argument("--fine-tune-lr", type=float, default=2e-6)
    parser.add_argument("--fine-tune-tail-blocks", type=int, default=6)
    parser.add_argument("--fine-tune-batch-size", type=int, default=16)
    parser.add_argument("--fine-tune-max-studies", type=int, default=0)
    parser.add_argument("--output", default=os.environ.get("RSNA_OUTPUT", "/kaggle/working/submission.csv"))
    args = parser.parse_args()
    if args.model_dir:
        os.environ["RSNA_DINOV2_MODEL_DIR"] = args.model_dir
    run(
        Path(args.data_dir),
        Path(args.output),
        visual_weight=args.visual_weight,
