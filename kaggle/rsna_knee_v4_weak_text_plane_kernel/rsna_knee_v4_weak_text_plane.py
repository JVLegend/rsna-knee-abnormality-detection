#!/usr/bin/env python3
"""H-33 — laudos weak no ramo textual e cabeça visual separada por plano. #RSNA #Kaggle #Dados
...
A diferença desta variante é que o ramo textual também recebe pseudo-rótulos weak
com confiança target-wise >= 0,85; os rótulos gold oficiais continuam prevalecendo.
...
def _text_training_frame(
    train: pd.DataFrame,
    teacher: pd.DataFrame | None,
    threshold: float = TEXT_WEAK_THRESHOLD,
) -> tuple[pd.DataFrame, int]:
    """Adiciona weak labels confidentes ao treino textual, preservando o gold."""

    if teacher is None:
        return train, 0
    if not 0.5 < threshold <= 1:
        raise ValueError("text_weak_threshold precisa estar em (0,5, 1].")
    if KEY_COLUMN not in teacher.columns:
        raise ValueError(f"Teacher textual precisa conter {KEY_COLUMN!r}.")
    keyed = teacher.copy()
    keyed[KEY_COLUMN] = keyed[KEY_COLUMN].astype(str)
    keyed = keyed.set_index(KEY_COLUMN)
    ids = train[KEY_COLUMN].astype(str)
    result = train.copy()
    weak_any = np.zeros(len(train), dtype=bool)
    for target in TARGET_COLUMNS:
        if target not in keyed.columns:
            raise ValueError(f"Teacher textual sem o alvo {target!r}.")
        official = pd.to_numeric(train[target], errors="coerce")
        probability = pd.to_numeric(ids.map(keyed[target]), errors="coerce")
        confidence_column = f"{target}__confidence"
        if confidence_column in keyed.columns:
            confidence = pd.to_numeric(ids.map(keyed[confidence_column]), errors="coerce")
        else:
            confidence = probability.sub(0.5).abs().mul(2.0)
        weak = official.isna() & probability.notna() & confidence.fillna(0).ge(threshold)
        labels = official.copy()
        labels.loc[weak] = (probability.loc[weak] >= 0.5).astype(float)
        result[target] = labels.to_numpy(dtype=float)
        weak_any |= weak.to_numpy(dtype=bool)
    return result, int(weak_any.sum())
...
def run(
    ...
    text_weak: bool = TEXT_WEAK_MODE,
    text_weak_threshold: float = TEXT_WEAK_THRESHOLD,
    ...
    if text_weak and not weak_visual:
        raise ValueError("text_weak=True exige weak_visual=True para carregar o teacher.")
    if not 0.5 < text_weak_threshold <= 1:
        raise ValueError("text_weak_threshold precisa estar em (0,5, 1].")
    if intensity_window not in {"slice", "series"}:
        raise ValueError(f"Janela de intensidade desconhecida: {intensity_window}")
    ...
    if weak_visual:
        ...
        if text_weak:
            text_train, text_weak_studies = _text_training_frame(
                train,
                teacher,
                threshold=text_weak_threshold,
            )
    text = text_predictions(text_train, test, train_series, test_series)
    ...
    print(
        ...
        f"weak_visual={weak_visual} weak_label_mode={weak_label_mode} "
        f"text_weak={text_weak} text_weak_studies={text_weak_studies} "
        f"external_labels={label_dir} "
...
def main() -> None:
    ...
    parser.add_argument("--visual-weight", type=float, default=0.1)
    ...
    parser.add_argument("--text-weak", dest="text_weak", action="store_true", default=TEXT_WEAK_MODE)
    parser.add_argument("--no-text-weak", dest="text_weak", action="store_false")
    parser.add_argument("--text-weak-threshold", type=float, default=TEXT_WEAK_THRESHOLD)
    ...
    run(
        ...
        text_weak=args.text_weak,
        text_weak_threshold=args.text_weak_threshold,
