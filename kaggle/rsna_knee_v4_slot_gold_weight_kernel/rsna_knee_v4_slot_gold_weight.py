#!/usr/bin/env python3
"""H-32 — H-28 com peso gold target-wise. #RSNA #Kaggle #Dados

Esta variante preserva a configuração H-28 (dense6, EfficientNet-B0,
teacher target-wise, pooling visual por alvo, blend texto-imagem, labels weak
suaves e janela 1–99% por série), mas seleciona uma série por cada slot
observado Sagittal/Coronal/Axial × FLUID_FS/NONFLUID. Cada slot recebe uma
cabeça visual própria e as probabilidades são agregadas apenas nos slots
presentes. Quando um slot não tem exemplos suficientes no treino, a cabeça
por plano da H-27 funciona como fallback explícito.

A única ablação adicional é o peso dos estudos gold: 8 nos 11 alvos e 1 em
Synovitis, conforme o gate local GroupKFold. O peso é aplicado tanto no
fine-tuning quanto nas cabeças visuais; texto, encoder, seleção de séries e
agregação permanecem iguais ao H-28.
...
GOLD_TARGET_WEIGHTS = {
    target: (1.0 if target == "Synovitis" else 8.0)
    for target in TARGET_COLUMNS
}
PLANE_NAMES = ("Sagittal", "Coronal", "Axial")
PLANE_KEYS = (*PLANE_NAMES, "Unknown")
SLOT_CATEGORIES = ("FLUID_FS", "NONFLUID")
SLOT_NAMES = tuple(f"{plane}_{category}" for plane in PLANE_NAMES for category in SLOT_CATEGORIES)
SLOT_KEYS = (*SLOT_NAMES, "Unknown")
LEXICON = {
...
VIEW_POOLING = os.environ.get("RSNA_VIEW_POOLING", "slot_target")
...
def _preferred_series_uids_by_slot(
    series: pd.DataFrame | dict[str, pd.DataFrame],
    study_uid: str,
) -> dict[str, str]:
    """Escolhe no máximo uma série por plano e categoria de aquisição."""
    ...
    candidates["_fat"] = pd.to_numeric(fat, errors="coerce").fillna(0)
    candidates["_plane_name"] = plane.fillna("").astype(str)
    candidates["_slot_category"] = np.select(
        [
            candidates["_fluid"].eq(1) & candidates["_fat"].eq(1),
            candidates["_fluid"].eq(0) & candidates["_fat"].eq(0),
        ],
        ["FLUID_FS", "NONFLUID"],
        default="Unknown",
    )
    ...
    for plane_name in PLANE_NAMES:
        for category in SLOT_CATEGORIES:
            subset = candidates[
                candidates["_plane_name"].eq(plane_name) & candidates["_slot_category"].eq(category)
            ].sort_values(["_fluid", "_fat", "SeriesInstanceUID"], ascending=[False, False, True])
            if not subset.empty:
                selected[f"{plane_name}_{category}"] = str(subset.iloc[0]["SeriesInstanceUID"])
    if selected:
        return selected
    fallback = candidates.sort_values(["_fluid", "_fat", "SeriesInstanceUID"], ascending=[False, False, True])
    return {"Unknown": str(fallback.iloc[0]["SeriesInstanceUID"])}
...
def study_images_by_slot(
    ...
    """Extrai views mantendo plano, categoria de aquisição e presença."""

    images_by_slot: dict[str, list[np.ndarray]] = {slot: [] for slot in SLOT_KEYS}
    for slot, series_uid in _preferred_series_uids_by_slot(series, study_uid).items():
        series_images, valid = _series_image(
            data_dir,
            split,
            study_uid,
        ...
        if valid:
            images_by_slot.setdefault(slot, []).extend(series_images)
    return images_by_slot, any(images_by_slot.values())
...
def _fine_tune_encoder(
    ...
    *,
    ...
    for target_index, target in enumerate(TARGET_COLUMNS):
        ...
        weights[:, target_index] = np.where(
            gold,
            GOLD_TARGET_WEIGHTS[target],
            source_weights,
        ).astype(np.float32)
...
def visual_embeddings(
    ...
    study_embeddings_by_slot: list[dict[str, list[np.ndarray]]] = [
        {slot: [] for slot in SLOT_KEYS} for _ in records
    ]
    mean = IMAGE_MEAN.to(device)
    std = IMAGE_STD.to(device)
    ...
    def flush() -> None:
        ...
        for owner, plane, slot, value in zip(image_owners, image_planes, image_slots, values, strict=True):
            study_embeddings[owner].append(value)
            study_embeddings_by_plane[owner].setdefault(plane, []).append(value)
            study_embeddings_by_slot[owner].setdefault(slot, []).append(value)
        images.clear()
        image_owners.clear()
        image_planes.clear()
        image_slots.clear()
    ...
    for position, (split, row, series) in enumerate(records, start=1):
        owner = position - 1
        study_images_for_row, valid = study_images_by_slot(
            data_dir,
            split,
            str(row[KEY_COLUMN]),
            series,
        ...
        for slot, slot_images in study_images_for_row.items():
            plane = slot.split("_", 1)[0] if slot in SLOT_NAMES else "Unknown"
    ...
    meta = {
        ...
        "slot_coverage_train": {
            slot: int(sum(bool(study_embeddings_by_slot[index].get(slot)) for index in range(train_count)))
            for slot in SLOT_NAMES
        },
        "slot_coverage_test": {
            slot: int(sum(bool(study_embeddings_by_slot[index].get(slot)) for index in range(train_count, len(records))))
            for slot in SLOT_NAMES
        },
        "valid_train_mask": valid[:train_count].tolist(),
        "series_selection": "one_best_series_per_anatomical_plane_and_observed_fluid_fs_category",
    ...
    return (
        ...
        study_embeddings_by_plane[train_count:],
        study_embeddings_by_slot[:train_count],
        study_embeddings_by_slot[train_count:],
    )
...
def _weak_target_arrays(
    ...
    for target in TARGET_COLUMNS:
        ...
        weights[gold] = GOLD_TARGET_WEIGHTS[target]
...
def _fit_target_slot_view_model(
    target: str,
    train_views_by_plane: list[dict[str, list[np.ndarray]]],
    test_views_by_plane: list[dict[str, list[np.ndarray]]],
    train_views_by_slot: list[dict[str, list[np.ndarray]]],
    test_views_by_slot: list[dict[str, list[np.ndarray]]],
    labels: np.ndarray,
    fit_mask: np.ndarray,
    sample_weights: np.ndarray | None,
    target_pooling: str | None = None,
    ...
    """Treina heads por slot e usa a cabeça por plano quando um slot é raro."""

    if len(train_views_by_slot) != len(labels) or len(train_views_by_slot) != len(fit_mask):
        raise ValueError("Os mapas de slot precisam coincidir com labels/fit_mask.")
    if len(test_views_by_slot) == 0:
        return np.empty(0, dtype=float)
    plane_predictions = _fit_target_plane_view_model(
        target,
    ...
    slot_predictions: dict[str, np.ndarray] = {}
    for slot in SLOT_NAMES:
        train_views = [study.get(slot, []) for study in train_views_by_slot]
        test_views = [study.get(slot, []) for study in test_views_by_slot]
        train_rows = [
            index for index, values in enumerate(train_views) if fit_mask[index] and values
        ]
        has_two_hard_classes = bool(train_rows) and np.unique(labels[train_rows]).size >= 2
        has_soft_signal = bool(
            soft_probabilities is not None
            and train_rows
            and np.isfinite(soft_probabilities[train_rows]).any()
        )
        if not train_rows or not (has_two_hard_classes or has_soft_signal):
            continue
        slot_predictions[slot] = _fit_target_view_model(
            target,
            train_views,
            test_views,
            labels,
    ...
    predictions: list[float] = []
    for index, study in enumerate(test_views_by_slot):
        present: list[float] = []
        for slot in SLOT_NAMES:
            if not study.get(slot):
                continue
            slot_prediction = slot_predictions.get(slot)
            present.append(float(plane_predictions[index] if slot_prediction is None else slot_prediction[index]))
        if not present:
            present = [float(plane_predictions[index])] if plane_predictions.size else [0.5]
        predictions.append(float(np.mean(present)))
    return np.clip(np.asarray(predictions, dtype=float), 1e-6, 1 - 1e-6)
...
def run(
    ...
    if slice_profile not in SLICE_PROFILES:
        raise ValueError(f"Perfil de fatias desconhecido: {slice_profile}")
    if view_pooling not in {"mean", "max", "topk", "target", "plane_target", "slot_target"}:
    ...
    embedding_pooling = "mean" if view_pooling in {"target", "plane_target", "slot_target"} else view_pooling
    (
        train_visual,
        test_visual,
        visual_meta,
    ...
    for target in TARGET_COLUMNS:
        if weak_visual:
            ...
            sample_weights = None
        if view_pooling == "slot_target":
            visual[target] = _fit_target_slot_view_model(
                target,
                train_views_by_plane,
                test_views_by_plane,
                train_views_by_slot,
        ...
        elif view_pooling == "plane_target":
...
def main() -> None:
    ...
    parser.add_argument("--view-pooling", choices=("mean", "max", "topk", "target", "plane_target", "slot_target"), default=VIEW_POOLING)
