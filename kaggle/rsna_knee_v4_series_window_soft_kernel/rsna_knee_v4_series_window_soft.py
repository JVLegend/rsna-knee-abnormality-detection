#!/usr/bin/env python3
"""H-22 — janela por série com labels weak suaves. #RSNA #Kaggle #Dados

Esta variante mantém exatamente a configuração do melhor kernel público local
(dense6, EfficientNet-B0, teacher target-wise, pooling visual por alvo, blend
texto-imagem e labels weak suaves) e troca somente a janela de intensidade:
as fatias usadas de cada série compartilham os percentis 1–99%.

O objetivo é testar se uma escala comum reduz diferenças artificiais de brilho
entre cortes de uma mesma aquisição sem introduzir qualquer dado do teste no
treino. O entrypoint é autocontido para Notebook-only: não clona o repositório,
não instala pacotes e não usa internet.

Para auditoria, os defaults abaixo reproduzem H-22. Argumentos de linha de
comando continuam disponíveis para smoke tests e comparação controlada.
"""
