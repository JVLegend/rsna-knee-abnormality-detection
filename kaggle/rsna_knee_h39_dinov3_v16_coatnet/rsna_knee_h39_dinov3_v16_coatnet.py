"""H-39: H-38 CoAtNet/DINOv3 blend plus the public DINOsaur V16 head.
...
per-target rank fusion. The final optional stage re-encodes the public six
slots once and applies the five-fold V16 SlotAttentionHead from the public
DINOsaur V4 train kernel, with its own OOF deployment gate. If that artifact
is not mounted or fails validation, the exact H-38 submission is retained.
"""
...
# H-39: public DINOsaur V16 target-conditioned residual.
# The train kernel is declared in kernel-metadata.json as a public kernel source.
# Its output is searched by contract rather than by a guessed mount path because
# Kaggle mounts Notebook Output Files differently between UI and API runs.
import math

def _h39_find_v16_bundle():
    matches = []
    root = Path('/kaggle/input')
    if root.is_dir():
        for current, dirs, files in os.walk(root):
            dirs[:] = [d for d in dirs if d not in ('train_series', 'test_series', '__pycache__')]
            if 'v16_manifest.json' in files:
                folder = Path(current)
                checkpoints = sorted(folder.glob('v16_slothead_f*.pt'))
                if len(checkpoints) >= 4:
                    matches.append((folder, checkpoints))
    matches.sort(key=lambda item: (len(item[0].parts), str(item[0])))
    return matches[0] if matches else (None, [])


class _H39V16SlotAttentionHead(nn.Module):
    def __init__(self, feature_dim, hidden=384, n_target=12, n_slot=6):
        ...
        self.slot_emb = nn.Parameter(torch.zeros(int(n_slot), int(hidden)))
        self.query = nn.Parameter(torch.zeros(int(n_target), int(hidden)))
        self.attn = nn.MultiheadAttention(
            int(hidden), num_heads=6, dropout=0.10, batch_first=True
        )
        self.fuse = nn.Sequential(
            nn.LayerNorm(int(hidden) * 5),
            nn.Linear(int(hidden) * 5, int(hidden)),
            nn.GELU(),
            nn.Dropout(0.16),
        )
        self.target_weight = nn.Parameter(torch.empty(int(n_target), int(hidden)))
        self.target_bias = nn.Parameter(torch.zeros(int(n_target)))

    def forward(self, features, slot_mask):
        mask = slot_mask.bool()
        x = self.proj(features.float()) + self.slot_emb.unsqueeze(0)
...
_H39_N_SLOT = len(_H39_SLOTS)
_H39_SIZE = 336
_H39_N_SLICE = 16
_H39_CROP_MM = 130.0
_H39_SLICE_BAND = (0.12, 0.88)


def _h39_read_crop(path):
    try:
        ds = pydicom.dcmread(path)
        arr = ds.pixel_array.astype(np.float32)
    except Exception:
    ...
    try:
        ...
        ps = _H39_CROP_MM / max(arr.shape)
    half = int(round(_H39_CROP_MM / ps / 2))
    cy, cx = (arr.shape[0] // 2, arr.shape[1] // 2)
    y0, y1 = (max(0, cy - half), min(arr.shape[0], cy + half))
    x0, x1 = (max(0, cx - half), min(arr.shape[1], cx + half))
    crop = arr[y0:y1, x0:x1]
...
def _h39_window(crop, lo, hi, flip):
    c = np.clip((crop - lo) / max(hi - lo, 1e-06), 0, 1)
    img = cv2.resize(c, (_H39_SIZE, _H39_SIZE), interpolation=cv2.INTER_AREA)
    return img[:, ::-1].copy() if flip else img


def _h39_render(path, flip):
    crop = _h39_read_crop(path)
    if crop is None:
        return None
    lo, hi = np.percentile(crop[::4, ::4], [1, 99])
    return _h39_window(crop, lo, hi, flip)


def _h39_build_study(args):
    """Decode the exact public V16 six-slot recipe after the CoAtNet globals are loaded."""
    idx, study, recs = args
    out = np.zeros((_H39_N_SLOT, _H39_N_SLICE, _H39_SIZE, _H39_SIZE), np.uint8)
    mask = np.zeros(_H39_N_SLOT, np.uint8)
    rows = pd.DataFrame(recs)
    if len(rows):
        for s_i, (plane, fs) in enumerate(_H39_SLOTS):
            sub = rows[(rows.Anatomical_Plane == plane) & (rows.Fat_Suppression == fs)]
            if sub.empty:
                continue
            files = ordered_files(SERIES_ROOT / study / sub.iloc[0].SeriesInstanceUID)
            if not files:
                continue
            flip = plane != 'Sagittal' and series_side(files[0]) < 0
            lo, hi = _H39_SLICE_BAND
            i0 = int(round(lo * (len(files) - 1)))
            i1 = int(round(hi * (len(files) - 1)))
            avail = list(range(i0, i1 + 1))
            if len(avail) >= _H39_N_SLICE:
                picks = [avail[int(round(t))] for t in np.linspace(0, len(avail) - 1, _H39_N_SLICE)]
                off = 0
            else:
                picks, off = (avail, (_H39_N_SLICE - len(avail)) // 2)
            for c, p in enumerate(picks):
                img = _h39_render(files[p], flip)
                if img is None:
                    img = _h39_render(files[min(len(files) - 1, p + 1)], flip)
                if img is not None:
                    out[s_i, off + c] = (img * 255).astype(np.uint8)
            mask[s_i] = len(picks)
    return (idx, out, mask)
...
@torch.no_grad()
def _h39_extract_v16_features(images, masks, feature_model, micro=8):
    records = []
    for study_index in range(len(masks)):
        for slot in np.nonzero(masks[study_index] > 0)[0]:
            records.append((study_index, int(slot)))
    if not records:
        raise RuntimeError('V16 test cache has no present slots')
    ...
    for start in range(0, len(records), micro):
        ...
        ims_np = np.stack([images[i, slot] for i, slot in part], axis=0)
        value = torch.from_numpy(ims_np).to(device).float().div_(255.0)
        slots = torch.tensor([slot + 1 for _, slot in part], device=device, dtype=torch.long)
        valid_mask = torch.from_numpy(
            ims_np.reshape(len(ims_np), ims_np.shape[1], -1).max(2) > 0
        ).to(device)
        x = value
        if feature_model.mixer is not None:
            x = feature_model.mixer(x, slots, valid_mask)
        if feature_model.compress is not None:
            x = feature_model.compress(x)
        with torch.autocast(
            'cuda', dtype=AMP_DT, enabled=AMP_ON
        ):
            if CFG['cond'] == 'token':
                tokens = feature_model.enc.forward_features(x, slots)
            else:
                tokens = feature_model.enc.forward_features(x)
        if tokens.ndim == 2:
            cls = mean = focal = tokens.float()
        else:
            prefix = int(getattr(feature_model.enc, '_orig_prefix', 1))
            cls = tokens[:, 0].float()
            patches = tokens[:, prefix:].float()
            mean = patches.mean(dim=1)
            grid = int(round(math.sqrt(patches.shape[1])))
            if grid * grid == patches.shape[1] and grid >= 3:
                patch_grid = patches.reshape(len(part), grid, grid, -1)
                radius = max(1, grid // 4)
                center = grid // 2
                focal = patch_grid[
                ...
                focal = mean
        feature_batches.append(torch.cat([cls, mean, focal], dim=1).cpu().numpy().astype(np.float16))
        index_batches.extend(part)

    feature_dim = feature_batches[0].shape[1]
    output = np.zeros((len(masks), _H39_N_SLOT, feature_dim), dtype=np.float16)
    output_mask = np.zeros((len(masks), _H39_N_SLOT), dtype=np.uint8)
    offset = 0
    for batch in feature_batches:
        for row in range(len(batch)):
            study_index, slot = index_batches[offset]
            output[study_index, slot] = batch[row]
            output_mask[study_index, slot] = 1
            offset += 1
    return output, output_mask


def _h39_run_v16():
    bundle, checkpoints = _h39_find_v16_bundle()
    if bundle is None:
        print('[H-39/V16] public train output not mounted; H-38 retained', flush=True)
        return False
    manifest = json.loads((bundle / 'v16_manifest.json').read_text(encoding='utf-8'))
    if manifest.get('artifact_type') != 'rsna-knee-dinov3-slothead-v16':
        raise RuntimeError('V16 artifact_type mismatch')
    if manifest.get('labels') != list(LABELS):
        raise RuntimeError('V16 label order mismatch')

    test_ids = [str(uid) for uid in studies]
    images = np.zeros((len(test_ids), _H39_N_SLOT, _H39_N_SLICE, _H39_SIZE, _H39_SIZE), dtype=np.uint8)
    masks = np.zeros((len(test_ids), _H39_N_SLOT), dtype=np.uint8)
    with ThreadPoolExecutor(max_workers=max(1, min(8, os.cpu_count() or 4))) as pool:
        futures = [
            pool.submit(_h39_build_study, (index, uid, by.get(uid, [])))
            for index, uid in enumerate(test_ids)
        ]
        for future in futures:
            index, array, mask = future.result()
            images[index] = array
            masks[index] = mask
    if not masks.any():
        raise RuntimeError('V16 test decode returned zero slots')

    feature_model = models[0].eval()
    device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
    feature_model = feature_model.to(device)
    features, feature_mask = _h39_extract_v16_features(images, masks, feature_model)
    if features.shape[-1] != int(manifest.get('feature_dim', features.shape[-1])):
        raise RuntimeError(
            f"V16 feature dimension mismatch: extracted={features.shape[-1]} manifest={manifest.get('feature_dim')}"
        )

    fold_stack = []
    feature_tensor = torch.from_numpy(features).to(device)
    mask_tensor = torch.from_numpy(feature_mask).to(device)
    for path in checkpoints:
        checkpoint = torch.load(path, map_location='cpu', weights_only=False)
        head = _H39V16SlotAttentionHead(
            checkpoint['feature_dim'],
            hidden=checkpoint.get('hidden', 384),
            n_target=checkpoint.get('n_target', len(LABELS)),
            n_slot=checkpoint.get('n_slot', N_SLOT),
        ).to(device).eval()
        head.load_state_dict(checkpoint['state_dict'], strict=True)
        values = []
        for start in range(0, len(features), 512):
            values.append(torch.sigmoid(head(
                feature_tensor[start:start + 512],
                mask_tensor[start:start + 512],
            )).detach().float().cpu().numpy())
        fold_stack.append(_h39_rank(np.concatenate(values, axis=0)))
        del head, checkpoint
        if device.type == 'cuda':
            torch.cuda.empty_cache()
    fold_stack = np.stack(fold_stack, axis=0)
    specialist = _h39_rank(fold_stack.mean(axis=0))

    anchor = pd.read_csv('/kaggle/working/submission.csv', dtype={'StudyInstanceUID': str})
    if anchor['StudyInstanceUID'].astype(str).tolist() != test_ids:
        raise RuntimeError('V16 anchor/test ID order mismatch')
    anchor_rank = anchor[LABELS].rank(method='average', pct=True).to_numpy(np.float64)
    stability = np.zeros(len(LABELS), dtype=np.float64)
    pair_count = 0
    for left in range(len(fold_stack)):
        for right in range(left + 1, len(fold_stack)):
            for target_index in range(len(LABELS)):
                x = fold_stack[left, :, target_index]
                y = fold_stack[right, :, target_index]
                if np.std(x) > 1e-12 and np.std(y) > 1e-12:
                    stability[target_index] += float(np.corrcoef(x, y)[0, 1])
            pair_count += 1
    stability /= max(pair_count, 1)

    target_cap = {
        'Medial Meniscus': 0.190,
        'Lateral Meniscus': 0.190,
        'Lateral OA': 0.040,
        'PF OA': 0.020,
        "Baker's": 0.030,
    }
    result = anchor.copy()
    promoted = []
    deploy = manifest.get('deploy_weights', {})
    validation = manifest.get('validation', {})
    for target_index, target in enumerate(LABELS):
        if target not in target_cap:
            continue
        train_weight = float(deploy.get(target, 0.0))
        info = validation.get(target, {})
        gain = float(info.get('nested_gain', 0.0))
        support = float(info.get('bootstrap_support', 0.0))
        if train_weight <= 0 or gain <= 0:
            continue
        gain_factor = float(np.clip(gain / 0.025, 0.25, 1.0))
        support_factor = float(np.clip((support - 0.55) / 0.35, 0.20, 1.0))
        evidence = 0.65 * gain_factor + 0.35 * support_factor
        stability_factor = float(np.clip((stability[target_index] - 0.45) / 0.35, 0.35, 1.0))
        corr = float(np.corrcoef(anchor_rank[:, target_index], specialist[:, target_index])[0, 1])
        if not np.isfinite(corr):
            diversity_factor = 0.50
        elif corr > 0.990:
            diversity_factor = 0.55
        elif corr > 0.975:
            diversity_factor = 0.80
        elif corr < 0.55:
            diversity_factor = 0.60
        else:
            diversity_factor = 1.00
        weight = float(np.clip(train_weight * evidence * stability_factor * diversity_factor, 0.0, target_cap[target]))
        if weight < 0.012:
            continue
        result[target] = pd.Series(
            (1.0 - weight) * anchor_rank[:, target_index] + weight * specialist[:, target_index]
        ).rank(method='average', pct=True).to_numpy(np.float64)
        promoted.append((target, weight, gain, support, stability[target_index], corr))

    if not promoted:
        print('[H-39/V16] no target passed OOF deployment gate; H-38 retained', flush=True)
        return False
    if not np.isfinite(result[LABELS].to_numpy()).all():
        raise RuntimeError('V16 produced non-finite submission values')
    result.to_csv('/kaggle/working/submission_v16.csv', index=False)
    result.to_csv('/kaggle/working/submission.csv', index=False)
    print('[H-39/V16] promoted: ' + '; '.join(
        f'{target}=w{weight:.3f},gain={gain:+.4f},sup={support:.3f},stab={stab:.3f},corr={corr:.3f}'
        for target, weight, gain, support, stab, corr in promoted
    ), flush=True)
    return True
...
try:
    _h39_run_v16()
except Exception as _h39_v16_error:
    print(
        '[H-39/V16] failed safely; exact H-38 submission retained: '
        f'{type(_h39_v16_error).__name__}: {_h39_v16_error}',
        flush=True,
    )

if not Path('/kaggle/working/submission.csv').is_file():
    raise RuntimeError('H-39 submission.csv missing after V16 stage')
