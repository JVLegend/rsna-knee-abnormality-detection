...
def visual_embeddings(
    ...
    for position, (split, row, series) in enumerate(records, start=1):
        image, valid = study_image(data_dir, split, str(row[KEY_COLUMN]), series)
        images.append(image)
        valid_flags.append(valid)
        if len(images) >= batch_size:
            flush()
        if position % 25 == 0 or position == len(records):
            print(f"visual={position}/{len(records)} valid={sum(valid_flags)}")
    flush()
...
def run(data_dir: Path, output: Path, visual_weight: float = 0.4, batch_size: int = 16, device: str = "auto") -> pd.DataFrame:
    ...
    for target in TARGET_COLUMNS:
        submission[target] = np.clip(
            (1 - visual_weight) * text[target].to_numpy(dtype=float)
            + visual_weight * visual[target].to_numpy(dtype=float),
            1e-6,
            1 - 1e-6,
        )
    ...
    print(f"labeled_train={len(train_labeled)} test={len(test)} visual_weight={visual_weight}")
...
def main() -> None:
    ...
    run(Path(args.data_dir), Path(args.output), visual_weight=args.visual_weight, batch_size=args.batch_size, device=args.device)
