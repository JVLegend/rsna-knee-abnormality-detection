...
# H-40: public WideDense v4 residual over the protected H-38 output.
# This is the complementary CoAtNet arm used by the public DINOsaur V10 family,
# attached here independently so a missing/incompatible arm cannot erase H-38.
def _h40_find_widedense_checkpoint():
    expected = 'raptor_ft_coatnet_v4_full_swa.pt'
    # Do not glob recursively under /kaggle/input: the competition mount may contain
    # the full train DICOM tree, turning a harmless lookup into an hours-long scan.
    roots = (
        Path('/kaggle/input/datasets/dreaddevelopment/raptor-knee-widedense'),
        Path('/kaggle/input/dreaddevelopment/raptor-knee-widedense'),
        Path('/kaggle/input/raptor-knee-widedense'),
    )
    for root in roots:
        path = root / expected
        if path.is_file():
...
def _h40_run_widedense():
    path = _h40_find_widedense_checkpoint()
    if path is None:
        print('[H-40/WideDense] public checkpoint not mounted; H-38 retained', flush=True)
        return False
    if not torch.cuda.is_available():
        print('[H-40/WideDense] CUDA unavailable; H-38 retained', flush=True)
        return False

    root = find_test_root()
    tsdir = os.path.join(root, 'test_series')
    test = pd.read_csv(os.path.join(root, 'test.csv'))
    test['StudyInstanceUID'] = test['StudyInstanceUID'].astype(str)
    test_ids = test['StudyInstanceUID'].tolist()
    tser = pd.read_csv(os.path.join(root, 'test_series.csv'))
    tser['StudyInstanceUID'] = tser['StudyInstanceUID'].astype(str)
    tser['SeriesInstanceUID'] = tser['SeriesInstanceUID'].astype(str)
    series_map = {key: value.to_dict('records') for key, value in tser.groupby('StudyInstanceUID')}
    reader = _make_reader()
    ...
    try:
        ...
        for study_index, study_id in enumerate(test_ids):
            try:
                primary_volume, primary_mask, _legacy_volume, _legacy_mask = build_study_pair(
                    study_id, series_map, tsdir, reader,
                )
                windows = eval_windows(
                    primary_volume,
                    primary_mask,
                ...
                prediction[study_index] = infer_probs(model, windows, device)
                success[study_index] = True
                del primary_volume, primary_mask, _legacy_volume, _legacy_mask, windows
            except Exception as error:
                print(
                    f'[H-40/WideDense] study {study_index} fallback: '
                    f'{type(error).__name__}: {error}',
                    flush=True,
                )
        coverage = float(success.mean())
        print(
            f'[H-40/WideDense] {coverage:.3f} coverage; '
            f'{path.name} | res={resolution}',
            flush=True,
        )
        if coverage < 0.98:
            return False

        wide_rank = rankpct(np.clip(prediction, 0, 1))
        anchor = pd.read_csv(_H40_ANCHOR_PATH, dtype={'StudyInstanceUID': str})
        if anchor['StudyInstanceUID'].astype(str).tolist() != test_ids:
            raise RuntimeError('WideDense anchor/test ID order mismatch')
        if list(anchor.columns[1:]) != list(LAB):
            raise RuntimeError('WideDense anchor label order mismatch')
        anchor_rank = rankpct(anchor[LAB].to_numpy(np.float64))
        weights = {
            'ACL': 0.030,
            'MCL': 0.120,
            'Medial Meniscus': 0.030,
            'Lateral Meniscus': 0.030,
            'Medial OA': 0.100,
            'Lateral OA': 0.030,
            'PF OA': 0.120,
            'Effusion': 0.100,
        ...
        output = anchor_rank.copy()
        promoted = []
        for target_index, target in enumerate(LAB):
            weight = float(weights[target])
            corr = float(np.corrcoef(anchor_rank[:, target_index], wide_rank[:, target_index])[0, 1])
            ...
            if weight < 0.015:
                continue
            output[:, target_index] = (
                (1.0 - weight) * anchor_rank[:, target_index]
                + weight * wide_rank[:, target_index]
            )
            promoted.append((target, weight, corr))
        output = rankpct(output)
        result = anchor.copy()
        result[LAB] = output
        result.to_csv('/kaggle/working/submission_widedense.csv', index=False)
        result.to_csv('/kaggle/working/submission.csv', index=False)
        print('[H-40/WideDense] promoted: ' + '; '.join(
            f'{target}=w{weight:.3f},corr={corr:.3f}'
            for target, weight, corr in promoted
        ), flush=True)
        return True
    finally:
        if model is not None:
...
_H40_ANCHOR_PATH = Path('/kaggle/working/submission_v53_main.csv')
if not _H40_ANCHOR_PATH.is_file():
    _H40_ANCHOR_PATH = _d42_primary
_H40_ANCHOR_BYTES = _H40_ANCHOR_PATH.read_bytes()
_d42_primary.write_bytes(_H40_ANCHOR_BYTES)
try:
    _h40_run_widedense()
except Exception as _h40_error:
    _d42_primary.write_bytes(_H40_ANCHOR_BYTES)
    print(
        '[H-40/WideDense] failed safely; H-38 retained: '
        f'{type(_h40_error).__name__}: {_h40_error}',
        flush=True,
    )

if not _d42_primary.is_file():
    raise RuntimeError('submission.csv missing after H-40 stage')
